...
_sage_const_1 = Integer(1); _sage_const_1337 = Integer(1337); _sage_const_0 = Integer(0); _sage_const_2 = Integer(2); _sage_const_5 = Integer(5); _sage_const_48763 = Integer(48763)
...
def inv(x, n):
    return pow(x, -_sage_const_1 , n)


def sol(r1, s1, h1, r2, s2, h2):
    numer = inv(s2, n) * h2 - _sage_const_1337  * inv(s1, n) * h1
    denom = _sage_const_1337  * inv(s1, n) * r1 - inv(s2, n) * r2
    ans = numer * inv(denom, n) % n
    print(ans)
    # F.<k1,k2,d> = GF(n)
    # F.<
    k1, k2, d = var("k1 k2 d")
    eq1 = k1 == k2 * _sage_const_1337 
    eq2 = k1 == s1 ** -_sage_const_1  * (h1 + d * r1)
    eq3 = k2 == s2 ** -_sage_const_1  * (h2 + d * r2)
    sol = solve([eq1, eq2, eq3], (k1, k2, d), domain=GF(n))
    print(sol)

    tmp = sol[_sage_const_0 ][_sage_const_2 ].rhs()
    print(type(tmp))
...
def main():
    pub = r.recvline().decode().strip()[_sage_const_5 :-_sage_const_1 ].split(", ")
    x, y = map(int, pub)
    hello = b"Hello, world!"
